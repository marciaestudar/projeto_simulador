...
Original file is located at
    https://colab.research.google.com/drive/1YAxV0P_d0plk4NH4KVTmdhbH4tgZ8uPe
"""
...
def falar(texto):
    ...
    try:
        tts = gTTS(text=texto, lang='pt', slow=False) # Adicionado slow=False para voz mais natural
        audio_file = "temp_audio.mp3"
        tts.save(audio_file)
        st.audio(audio_file, format='audio/mp3', start_time=0, loop=False) # loop=False é o padrão, mas bom explicitar
        # Pequeno atraso para garantir que o áudio seja tocado antes da remoção
        time.sleep(0.5)
        if os.path.exists(audio_file):
            os.remove(audio_file) # Remove o arquivo temporário após tocar
    except Exception as e:
        # st.error(f"Erro ao gerar áudio: {e}. Verifique sua conexão com a internet ou as configurações do gTTS.")
        # Mensagem de erro mais amigável para o usuário
        st.warning("Não foi possível reproduzir o áudio. Verifique sua conexão com a internet.")
...
def verificar_resposta(resposta_usuario):
    ...
    st.session_state.input_resposta_desabilitado = True # Desabilita o input após a primeira tentativa
    ...
    elif resposta_usuario is None: # Se a resposta é None, significa que o tempo acabou ou não foi digitado nada
        st.session_state.erros += 1
        st.session_state.feedback_placeholder.error(
    ...
    # Fala "Próxima" após o feedback da resposta, mas antes de avançar para a próxima tela
    falar("Próxima")
...
if not st.session_state.iniciado:
    ...
    if st.session_state.tempo_inicio_pergunta and not st.session_state.mostrar_resultado_pergunta:
        tempo_decorrido = time.time() - st.session_state.tempo_inicio_pergunta
        ...
        if tempo_restante == 0 and not st.session_state.input_resposta_desabilitado:
            ...
            st.rerun() # Força o Streamlit a re-executar para atualizar o estado
    ...
    with col1:
        ...
        resposta_usuario = st.number_input(
            ...
            key=f"resposta_input_{st.session_state.perguntas_feitas}_{st.session_state.last_key}", # Chave única para resetar
    ...
    with col2:
        ...
        if st.button("Responder", disabled=st.session_state.input_resposta_desabilitado):
            # O number_input pode retornar 0 se vazio e o usuário clicar em responder.
            # Verificamos se há um valor válido antes de processar.
            if resposta_usuario is not None:
                verificar_resposta(int(resposta_usuario))
                st.rerun() # Força o Streamlit a re-executar para atualizar o estado
            else:
                st.session_state.feedback_placeholder.warning("Por favor, digite sua resposta.")
