...
Original file is located at
    https://colab.research.google.com/drive/1VD7gmQC1SY8HEXGFFkGUVfuVPIRFq5QP
"""
...
def iniciar_maratona():
    ...
    # Inicializa as respostas do usuário como None para cada pergunta
...
def verificar_todas_respostas():
    ...
    for pergunta_obj in st.session_state.perguntas:
        ...
        # Pega a resposta do usuário do dicionário (string do text_input)
        resposta_do_campo_str = st.session_state.respostas_usuario.get(pergunta_id)
...
if not st.session_state.iniciado:
    ...
    for i, pergunta_obj in enumerate(st.session_state.perguntas):
        ...
        with cols[col_index]:
            ...
            # Usamos st.text_input para remover os botões +/-
            # O 'type="number"' ajuda a exibir o teclado numérico em celulares
            # on_change é usado para atualizar st.session_state.respostas_usuario imediatamente
            # O valor inicial do text_input é obtido do session_state, ou uma string vazia se não houver
            st.session_state.respostas_usuario[pergunta_obj['id']] = st.text_input(
                "Sua resposta:",
                key=f"resposta_input_{pergunta_obj['id']}_{st.session_state.last_key}",
                value=str(st.session_state.respostas_usuario.get(pergunta_obj['id'], '')), # Converte para string para o text_input
                type="number", # Sugere teclado numérico
                max_chars=3 # Limita a 3 caracteres para respostas de tabuada (máx 9x10=90, 9x9=81)
            )
